...
int main() {
    ...
    // CubeScanner Testing ----------------------------------------------------------------------------------
    string fileName = "C:\\Users\\malim\\CLionProjects\\rubiks-cube-solver\\Databases\\cornerDepth5V1.txt";

    try {
        // 1. Initialize scanner (use camera index 0)
        CubeScanner scanner(0);

        // 2. Create cube object
        RubiksCubeBitboard cube;

        cout << "📷 Please show each face of the cube to the camera...\n";

        // 3. Scan cube
        scanner.scan(cube);

        // 4. Print scanned cube
        cout << "\nScanned Cube:\n";
        cube.print();

        // 5. Solve with IDA*
        IDAstarSolver<RubiksCubeBitboard, HashBitboard> idaStarSolver(cube, fileName);
        auto solveMoves = idaStarSolver.solve();

        // 6. Print solution
        cout << "\nSolution:\n";
        for (auto m : solveMoves) cout << cube.getMove(m) << " ";
        cout << "\n\nSolved cube:\n";
        idaStarSolver.rubiksCube.print();
    ...
    //try {
    //     // Create scanner using default webcam (index 0)
    //     CubeScanner scanner(0);
    //
    //     // Use the Bitboard cube representation (needed for solver)
    //     RubiksCubeBitboard cube;
    //
    //     // Scan the real cube
    //     scanner.scan(cube);
